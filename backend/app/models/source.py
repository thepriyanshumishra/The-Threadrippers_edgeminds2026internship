...
class SourceBase(BaseModel):
    name: str = Field(
        ..., description="Name of the source file, video title, or website title"
    )
    type: str = Field(
        ...,
        description="Type of source (pdf, image, audio, youtube, website, text, email)",
    )
...
class Source(SourceBase):
    ...
    status: str = Field(
        "pending", description="Processing status (pending, processing, ready, failed)"
    )
    summary: Optional[str] = Field(
        None, description="Extracted textual summary or preview"
    )
    stats: Optional[dict] = Field(
        None, description="Extracted statistics (pages, words, chunks)"
    )
