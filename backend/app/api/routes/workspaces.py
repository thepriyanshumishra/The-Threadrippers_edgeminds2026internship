...
def sanitize_workspace_name(name: str) -> str:
    """Strip path traversal characters and control chars from workspace names."""
...
def create_workspace(payload: WorkspaceCreate):
    ...
    metadata = Workspace(
        id=workspace_id,
        name=sanitize_workspace_name(payload.name),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create workspace storage"
        )
...
def update_workspace(
    ...
    try:
        ...
        if payload.name is not None:
            workspace.name = sanitize_workspace_name(payload.name)
        ...
        logger.info(
            f"Workspace {workspace_id} updated. Name: {workspace.name}, Instructions: {workspace.instructions}"
        )
        return workspace
    except Exception as e:
        logger.error(f"Failed to update workspace {workspace_id}: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to update workspace metadata"
        )
...
def delete_workspace(
    ...
    try:
        ...
        if workspace_id in processing_jobs:
            ...
            if "cancel_event" in job:
                ...
                logger.info(
                    f"Cancelled active processing job for deleted workspace {workspace_id}"
                )
            processing_jobs.pop(workspace_id, None)
    except Exception as e:
        logger.error(
            f"Error cancelling active processing job for workspace {workspace_id}: {e}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete workspace storage"
        )
...
def get_workspace_stats(
    ...
    if db_path.exists():
        try:
            ...
            logger.error(
                f"Failed to query chunk counts for workspace {workspace_id}: {e}"
            )
