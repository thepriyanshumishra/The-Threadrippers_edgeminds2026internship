...
class AudioProcessor:
    ...
    def create_chunks_from_segments(
        self, segments: List[Any], duration: float, workspace_id: str, source_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for seg in segments:
            ...
            if text.strip():
                uniform_segments.append({"text": text, "start": start, "end": end})
        ...
        for segment in uniform_segments:
            ...
            char_ranges.append((start_idx, end_idx, segment["start"], segment["end"]))
        ...
        def get_timestamps_for_range(start_char: int, end_char: int) -> tuple:
            ...
            for start_idx, end_idx, start_time, end_time in char_ranges:
                if start_idx <= start_char < end_idx or (
                    chunk_start is None and start_idx >= start_char
                ):
        ...
        if clean_text:
            ...
            for start_idx, end_idx, parent_idx in child_boundaries:
                ...
                if chunk_text:
                    chunk_start, chunk_end = get_timestamps_for_range(
                        start_idx, end_idx
                    )
                    child_chunks.append(
                        {
        ...
        logger.info(
            f"Generated {len(child_chunks)} child chunks, {len(parent_texts)} parent chunks, {total_words} words for source {source_id}."
        )
        return {
            "stats": {
                "duration": duration,
    ...
    def process(
        ...
        try:
            ...
            logger.info(
                f"Loading faster-whisper model '{settings.whisper_model}' (INT8 CPU)..."
            )
            model = WhisperModel(
                settings.whisper_model, device="cpu", compute_type="int8", cpu_threads=4
            )
            ...
            logger.info(
                f"Transcription finished. Duration: {duration:.2f}s, Language: {info.language}"
            )
        ...
        return self.create_chunks_from_segments(
