...
def clean_vtt_timestamps(vtt_text: str) -> List[Dict[str, Any]]:
    ...
    if current_text and current_start is not None:
        ...
        if text_content:
            parsed_segments.append(
                {"start": current_start, "end": current_end, "text": text_content}
            )
...
class YouTubeProcessor:
    ...
    def process(
        self, url: str, workspace_id: str, source_id: str, sources_dir: Path
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Available subtitle languages for '{video_title}': {available_langs}"
            )
            ...
            if selected_lang:
                ...
                ydl_opts_subs = {
                    ...
                    "subtitleslangs": [
                        selected_lang
                    ],  # Download ONLY the selected language
                ...
                downloaded_files = glob.glob(
                    str(sources_dir / f"{source_id}_sub_temp*")
                )
                ...
                if vtt_files:
                    ...
                    try:
                        ...
                        if segments:
                            ...
                            logger.info(
                                f"Successfully fetched {len(segments)} subtitle segments for YouTube video: '{video_title}'"
                            )
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to delete temp subtitle file {vtt_file}: {del_err}"
                            )
            else:
                logger.info(
                    "No subtitles or automatic captions available for this video."
                )
        ...
        if subs_downloaded and segments:
            res = self.audio_processor.create_chunks_from_segments(
                segments, duration, workspace_id, source_id
            )
            return {
                "title": video_title,
                "stats": res["stats"],
        ...
        logger.info(
            f"Subtitles not available for '{video_title}'. Falling back to local AI transcription..."
        )
        ...
        try:
            ...
            if not real_audio_path.exists():
                raise FileNotFoundError(
                    f"Extracted YouTube audio file not found at {real_audio_path}"
                )

            logger.info(
                "Delegating transcription of downloaded YouTube stream to AudioProcessor..."
            )
            ...
            for filepath in glob.glob(str(sources_dir / f"{source_id}_temp*")):
                try:
                    ...
                    logger.error(
                        f"Failed to delete temporary YouTube file {filepath}: {del_err}"
                    )

        return {"title": video_title, "stats": res["stats"], "summary": res["summary"]}
