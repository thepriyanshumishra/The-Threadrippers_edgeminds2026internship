...
class PDFProcessor:
    ...
    def process(
        ...
        try:
            ...
            raise ValueError(
                f"Failed to open PDF file. The file may be corrupted or encrypted. Details: {e}"
            )
        ...
        if total_chars == 0:
            doc.close()
            logger.warning(
                f"PDF file {file_path} contains no readable text (could be scanned document or empty)."
            )
            # Return empty stats instead of crashing
            return {
                "stats": {"pages": page_count, "words": 0, "chunks": 0},
        ...
        for page_text in pages_text:
            if page_text.strip():
                ...
                if len(preview_text) > 300:
                    break
        summary = preview_text[:300].strip() + (
            "..." if len(preview_text) > 300 else ""
        )

        doc.close()

        logger.info(
            f"PDF processed: {page_count} pages, {total_words} words, {len(child_chunks)} child chunks, {len(parent_texts)} parent chunks generated."
        )
        return {
            "stats": {
                "pages": page_count,
