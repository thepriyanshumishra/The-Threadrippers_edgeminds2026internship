    ...
    while idx < len(spans):
        ...
        if span_len > chunk_size:
            ...
            while w_idx < len(word_spans):
                ...
                if (
                    current_len + w_len + (1 if current_chunk_spans else 0) > chunk_size
                    and current_chunk_spans
                ):
                    chunks.append(
                        (current_chunk_spans[0][0], current_chunk_spans[-1][1])
                    )
                    overlap_spans = []
                    overlap_len = 0
                    for os in reversed(current_chunk_spans):
                        os_len = os[1] - os[0]
                        if (
                            overlap_len + os_len + (1 if overlap_spans else 0)
                            <= chunk_overlap
                        ):
        ...
        if (
            current_len + span_len + (1 if current_chunk_spans else 0) > chunk_size
            and current_chunk_spans
        ):
            chunks.append((current_chunk_spans[0][0], current_chunk_spans[-1][1]))
            overlap_spans = []
...
def find_parent_child_boundaries(
    text: str,
...
class TextProcessor:
    ...
    def process(
                ...
                child_chunks.append(
                    {
                        "index": idx,
                        "text": chunk_text,
                        "metadata": {"source": "pasted_text", "parent_id": parent_idx},
                    }
                )
        ...
        logger.info(
            f"Text processed: {total_words} words, {len(child_chunks)} child chunks, {len(parent_texts)} parent chunks generated."
        )
